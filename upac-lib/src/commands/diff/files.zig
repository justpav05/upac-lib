...
pub fn diffFiles(repo_path: []const u8, from_ref: []const u8, to_ref: []const u8, root_path: []const u8, allocator: std.mem.Allocator) ![]DiffEntry {
    ...
    const from_checkout_path = try std.fmt.allocPrintZ(allocator, "{s}/tmp/upac_diff_from_{d}", .{ root_path, timestamp });
    defer allocator.free(from_checkout_path);
    defer std.fs.deleteTreeAbsolute(from_checkout_path) catch {};

    const to_checkout_path = try std.fmt.allocPrintZ(allocator, "{s}/tmp/upac_diff_to_{d}", .{ root_path, timestamp + 1 });
    defer allocator.free(to_checkout_path);
    defer std.fs.deleteTreeAbsolute(to_checkout_path) catch {};

    try checkoutRef(repo.?, from_ref_c, from_checkout_path);
    try checkoutRef(repo.?, to_ref_c, to_checkout_path);
    ...
    if (c_libs.ostree_diff_dirs(c_libs.OSTREE_DIFF_FLAGS_NONE, from_gfile, to_gfile, modified_entries, removed_entries, added_entries, null, &gerror) == 0) return diff.DiffError.DiffFailed;
...
pub fn diffFilesAttributed(repo_path: []const u8, from_ref: []const u8, to_ref: []const u8, root_path: []const u8, db_path: []const u8, allocator: std.mem.Allocator) ![]AttributedDiffEntry {
    const raw_diff = try diffFiles(repo_path, from_ref, to_ref, root_path, allocator);
    defer {
...
// ── Private helpers ───────────────────────────────────────────────────────────
fn checkoutRef(repo: *c_libs.OstreeRepo, ref_c: [:0]const u8, destination_path: [:0]const u8) !void {
    var gerror: ?*c_libs.GError = null;
    defer if (gerror) |err| c_libs.g_error_free(err);
    ...
    if (c_libs.ostree_repo_checkout_at(repo, &options, std.c.AT.FDCWD, destination_path.ptr, resolved_checksum, null, &gerror) == 0) return diff.DiffError.DiffFailed;
}
...
fn buildFilePkgMap(repo_path: []const u8, ref: []const u8, db_path: []const u8, out: *std.StringHashMap([]const u8), allocator: std.mem.Allocator) !void {
    ...
    if (c_libs.ostree_repo_open(repo, null, &gerror) == 0) return;

    const body = (try packages.getRefBody(repo.?, ref, allocator)) orelse return;
    ...
    while (iter.next()) |entry| {
        ...
        while (file_map_iter.next()) |file_entry| {
            if (!out.contains(file_entry.key_ptr.*)) {
                try out.put(
                    try allocator.dupe(u8, file_entry.key_ptr.*),
